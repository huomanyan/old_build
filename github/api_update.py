...
update_sql = "update api set stargazers_count=%s,watchers_count=%s,forks_count=%s,network_count=%s,subscribers_count=%s where repo_id=%s"


def api_get():
    try:
        repo = r.spop("project_issues").decode('utf-8')
        ...
        while (True):
            access_token = (r.rpop("access_token_hmy")).decode('utf-8')
            ...
            try:
                ...
                if json_str.__contains__("message"):
                    ...
                    #todo
                    # 所有的0改为 f20171101，以防新爬取的一页得到的为空值，覆盖了前面所有
                ...
                # 插入值并更新标志位
                ver.execute(update_sql%(stargazers_count,watchers_count,forks_count,network_count,subscribers_count,repo_id))
