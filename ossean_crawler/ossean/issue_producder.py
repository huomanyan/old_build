...
conn.autocommit(1)
cursor = conn.cursor()
forks_select="select id,url from osp_github_total where id in (select id from ossean_metric WHERE ossean_metric.issue_close is null)"
r.delete('issues_set')
def start():
