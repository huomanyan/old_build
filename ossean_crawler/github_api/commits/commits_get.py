...
format_commits= "%s/commits?page=%s&&access_token=%s&&per_page=100&&state=all"
...
insert_commits= "insert ignore into commits(repo_id,created_at,author,committer,sha) values(%s,%s,%s,%s,%s)"
...
def commits_get():
    ...
    while (True):
        ...
        it_url= format_commits % (url, page_num, access_token)
        it_c = pycurl.Curl()
            ...
            if (type(it_hhh) == dict):
                ...
                if(len(it_hhh)==0):
                    ...
                    for it_ii in it_hhh:
                        ...
                        created_at =None if it_ii["commit"]["author"]["date"] is None else it_ii["commit"]["author"]["date"]
                        ...
                        if it_ii["committer"]==None:
                            ...
                            if 'login' in it_ii["committer"]:
                                ...
                                committer = None
                                print url,sha
                        cur.execute(insert_commits, (repo_id, created_at, author,committer,sha))
                page_num += 1
            if(commits_tag==0):
                break
    if commits_tag == 0 :
