...
# @File: get_article_urls_by_dynamic_loading.py
# @Description: 文章链接爬取（动态加载的网站）
# @Time: 2018/12/4 20:11
# @Author: ljz
...
class Crawler:
    def __init__(self, page_name, page_url, request_method, request_param, page_is_end,
        ...
        :param request_method: 网页请求方式
        :param request_param: 网页参数
    ...
    def parse_web_page(self, page_encoding='utf-8'):
        ...
        while running_flag:
            page_number += 1
            request_param = ast.literal_eval(self._request_param.format(page_number))

            if self._request_method == 'get':
                if self._page_is_end == '404':
                    if judge_page_is_end.error_404(self._page_url, param=request_param):
                        break
                else:
                    if judge_page_is_end.no_data(self._page_url, self._page_is_end, param=request_param):
                        break
                page_request = requests.get(self._page_url, params=request_param, headers=self._headers)
            else:
                if self._page_is_end == '404':
                    if judge_page_is_end.error_404(self._page_url, 'post', request_param):
                        break
                else:
                    if judge_page_is_end.no_data(self._page_url, self._page_is_end, 'post', request_param):
                        break
                page_request = requests.post(self._page_url, data=request_param, headers=self._headers)
