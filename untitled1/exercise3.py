...
X1 = tf.placeholder(dtype='float', shape=[None, 392], name='x1')
X2 = tf.placeholder(dtype='float', shape=[None, 392], name='x2')
...
with tf.Session() as sess:
    ...
    for i in range(step):
        ...
        _, loss, pred = sess.run([train_step, cross_entropy, y], feed_dict={X1:batch_xs[:,0:392], X2:batch_xs[:,392:784], y_:batch_ys})
        if i%100==0:
    ...
    accuracy = tf.reduce_mean(tf.cast(correct_prediction, 'float'))
    print('[accuracy, loss]]:', sess.run([accuracy, cross_entropy], feed_dict={X1:mnist.test.images[:,0:392], X2:mnist.test.images[:,392:784], y_:mnist.test.labels}))
